<template>
  <div class="checkbox-outline" v-loading="treeLoading" element-loading-spinner="car-loading">
    ...
    <div class="treeBox">
      <div class="tree-select" v-for="(tree, index) in treeData" :key="tree.outlineId">
        <div class="top-node">
          <el-row class="node-row">
            ...
            <el-col :span="2">
              <i class="show-icon el-icon-arrow-down" :class="{'expend': checkAllNode[index].expend}" @click="checkAllNode[index].expend = !checkAllNode[index].expend"></i>
            </el-col>
          </el-row>
        </div>
        <el-tree v-show="checkAllNode[index].expend" ref="outlineTree" class="filter-tree" :data="tree.children" :props="treeProps" :filter-node-method="filterNode" node-key="outlineId" show-checkbox @check="handleCheck">
