<template>
    <div class="question-item">
        <div class="question-stem" onselectstart="return false" oncopy="return false">
            <el-row>
                    ...
                    <div class="stem-info">
                        ...
                        <div class="question-cnt">
                            ...
                            <div v-if="question.questionTypeName === '单选题' || question.questionTypeName === '判断题'">
                                <div v-for="(item, index) in question.listPo" :key="item.pqOptionId"
                                    class="question-option">
                                    <div class="answer-radio"
                                        :style="{fontSize: fontSize + 'px', lineHeight: fontSize + 'px' }"
                                        @click="answer = index;changeSelect(item,'1')">
                                        <i class="radio-icon" :class="{'el-icon-success': answer === index}" :style="{
                        ...
                        height: fontSize > 18 ? fontSize + 'px' : '18px'}"></i>
                            ...
                            <div v-if="question.questionTypeName === '多选题'">
                                <el-checkbox-group v-model="defaultChecked">
                                    <div v-for="item in question.listPo" :key="item.pqOptionId" class="question-option">
                                        <el-checkbox class="checkbox-option" :label="item.pqOptionId"
                                            :style="{fontSize: fontSize + 'px', lineHeight: fontSize + 'px'}"
                            ...
                            <div v-if="question.questionTypeName === '简答题' || question.questionTypeName === '论述题'"
                                style="width: 100%;">
                                <el-input type="textarea" :rows="8" cols="100%" placeholder="请输入答案"
                                    v-model="question.answer" @change="changeSelect(question, '3')"></el-input>
                            </div>
                        </div>
                    </div>
                </el-col>
                ...
                if (this.question.listPo && this.question.listPo.length) {
                    this.question.listPo.forEach((item, index) => {
                        ...
                        if (answer.length && answer.indexOf(item.optionNum) > -1) {
                            item.checked = true;
                            item.optionKey = '1';
                            if (this.question.questionTypeName === '单选题' || this.question.questionTypeName ===
                                '判断题') {
                                this.answer = index;
                            }
