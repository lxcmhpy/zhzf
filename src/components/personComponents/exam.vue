  ...
  <div>
    <div>
      <div class="card-title">
        ...
        <el-button v-if="params.type !== 'view'" type="primary" style="margin-right:25px;float:right;" icon="el-icon-plus" size="cats-common" @click="addExam">新增考试</el-button>
      </div>
      <el-table style="margin-left:25px;width:97%;margin-bottom:35px;" :data="tableData" resizable stripe>
  ...
  methods: {
    ...
    deleteExam(row) {
      ...
      this.$confirm('确定要删除吗?', '提示', {
        ...
        .then(() => {
          _this.$store.dispatch('deleteExamInfoByIdMoudle', row.examId).then(
            res => {
              if (res.code === 200) {
                _this.catsMessage({
                  type: 'success',
