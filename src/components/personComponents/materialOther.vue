<template>
    <div style="width:100%;float:left;margin-top:30px;margin-bottom:30px;">
        ...
        <div ref="otherXX" class="block" style="margin-left:20px;float:left">
            <el-upload
                action="#"
                multiple
    ...
    methods:{
        handleChange(file,fileList){
            ...
            if(fileList.length>0){//已经上传了文件
                _this.$refs.otherXX.children[0].children[1].style="margin-top:270px;"
            }else{
                _this.$refs.otherXX.children[0].children[1].style="margin-top:-10px;"
            }
            if(fileList.length>1){//文件上传了多个时
                for(var i=0;i<fileList.length-1;i++){
                    _this.$refs.otherXX.children[0].children[0].children[i].style="margin-right:60px;"
                }
            }
        },
