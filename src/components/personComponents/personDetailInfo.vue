...
<div class="pserson-detail-wrap">
  <div class="new_css" v-show="isShowAddPerson">
    <el-form ref="addForm" :model="personInfoDetailForm" label-width="140px" :rules="rules">
      <div class="float css_left" style="position: absolute;top:0;bottom:0;"><!-- 左侧内容 -->
        <div ref="uploadRef" class="block" style="margin-top:50px;">
            <div class="image-view">
              <img :src="personImg" alt="" />
            </div>
            <div class="el-upload--picture-card">
              <el-button size="medium" @click="uploadAvatar">上传照片</el-button>
            </div>
        </div>
      </div>
                ...
                <el-col :span="12">
                  <el-form-item label="政治面貌" prop="politicalStatusName">
                    ...
                    <el-select
                      ...
                      <el-option
                          v-for="value in politicalInfo" :key="value.id" :label="value.name" :value="value.id">
                      </el-option>        
                    </el-select>
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="12">
                  <el-form-item label="分配渠道" prop="disChannel">
                    ...
                    <el-select
                      ...
                      <el-option
                        v-for="value in disChannelInfo" :key="value.id" :label="value.name" :value="value.id">
                      </el-option>        
                    </el-select>
                  </el-form-item>
                </el-col>
                  ...
                  <div v-if="pageParams.type">
                    <el-form-item label="所属机构" prop="oid">
                      <el-select disabled v-model="personInfoDetailForm.oid" placeholder="所属机构">
                        <el-option
                          :key="personInfoDetailForm.oid" :label="personInfoDetailForm.oname" :value="personInfoDetailForm.oid">
                        </el-option>  
                      </el-select>
                    </el-form-item>
                  </div>
                  ...
                  <el-form-item label="人员编制" prop="staffingName">
                    ...
                    <el-select
                      ...
                      @focus="getDictInfo('人员信息-人员编制','staffingInfo')">
                        <el-option
                            v-for="value in staffingInfo" :key="value.id" :label="value.name" :value="value.id">
                        </el-option>        
                    </el-select>
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="12">
                  <el-form-item label="职务" prop="postName">
                    ...
                    <el-select
                      ...
                      <el-option
                        v-for="value in postInfo" :key="value.id" :label="value.name" :value="value.id">
                      </el-option>        
                    </el-select>
                  </el-form-item>
                </el-col>
                  ...
                  <el-form-item label="岗位" prop="stationId">
                    ...
                    <el-select
                      ...
                      @change="selectVal($event,'stationId')">
                        <el-option
                            v-for="value in stationInfo" :key="value.id" :label="value.name" :value="value.id">
                        </el-option>        
                    </el-select>
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="12">
                  <el-form-item
                    ...
                    <el-select
                      ...
                      <el-option
                          v-for="value in branchInfo" :key="value.id" :label="value.name" :value="value.id">
                      </el-option>        
                    </el-select>
                  </el-form-item>
                </el-col>
              ...
              <el-row>
                <el-col :span="12">
                  <el-form-item label="从事执法日期" prop="enfoceDate">
                    ...
                    <el-date-picker v-model="personInfoDetailForm.enfoceDate" format="yyyy-MM-dd" value-format="yyyy-MM-dd"
                      placeholder="请选择从事执法日期" clearable>
                    </el-date-picker>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
        ...
        <div class="info_box">
          ...
          <div class="info_content">
            ...
            <div class="info_body">
              ...
              <el-row>
                <el-col :span="12">
                  <el-form-item label="从事执法日期：" prop="enfoceDate">{{personInfoDetailForm.enfoceDate}}</el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="法律职业资格证号：" prop="lawCertNo">{{personInfoDetailForm.lawCertNo}}</el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="12">
                  <el-form-item label="部级执法证号：" prop="ministerialNo">{{personInfoDetailForm.ministerialNo}}</el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="12">
                  <el-form-item label="省内执法证号：" prop="provinceNo">{{personInfoDetailForm.provinceNo}}</el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="海事执法证号：" prop="maritimeNo">{{personInfoDetailForm.maritimeNo}}</el-form-item>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="12">
                  <el-form-item label="证件办理状态：" prop="certStatusName">{{personInfoDetailForm.certStatusName}}</el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="当前证件状态：" prop="currentCertStatusName">{{personInfoDetailForm.currentCertStatusName}}</el-form-item>
                </el-col>
              </el-row>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        idNo: [{ required: true, message: "身份证号必须填写", trigger: "blur" },{validator: this.vailidIdNo,trigger:"blur"}],
        nationName: [{ required: true, message: "民族必须填写", trigger: "change" }],
        workDate: [{ required: true, message: "参加工作时间必须填写", trigger: "blur" }],
        politicalStatusName: [{ required: true, message: "政治面貌必须填写", trigger: "change" }],
        degree: [{ required: true, message: "学历必须填写", trigger: "change" }],
        postName: [{ required: true, message: "职务必须填写", trigger: "change" }],
        stationId: [{ required: true, message: "岗位必须填写", trigger: "change" }],
        lawCertNo: [{ validator:this.validateLawCertNo,trigger:"blur"}],
        oid: [{ required: true, message: "所属机构必须填写", trigger: "change" }],
        staffingName: [{ required: true, message: "人员编制必须填写", trigger: "change" }]
      },
  ...
  components:{
    elSelectTree, UploadPersonAvatar
  },
  created(){
    this.searchTable();
  ...
  methods: {
    ...
    stationChange(stationId){
      ...
      if(station){
        if(station.isLce === '0'){
          this.rules['branchName'] = [{ required: true, message: "执法领域必须填写", trigger: "change" }]
        }
      }
    ...
    saveEditConfirm(messageName, methodSaveOrUpdate) {
      const _this = this;
      if(_this.pageParams.type == 'edit'){//修改的时候提示
        let confirmText = '';
    ...
    submitPersonInfo(messageName, methodSaveOrUpdate) {
      ...
      _this.$store.dispatch(methodSaveOrUpdate, _this.personInfoDetailForm).then(res => {
        if(res.code===200){
          _this.$message({
            type: "success",
            message: messageName
    ...
    // 修改基本信息
    editPersonPage(){
      this.$store.dispatch('personEdabled', {personId: this.pageParams.id}).then(res => {
        if(res.code === 200){
          this.isShowAddPerson = true;
          this.showSelectTree = true;
          this.pageType = '0';
          this.stationChange(this.personInfoDetailForm.stationId);
        }else{
          this.$message({type: 'warning', message: '该人员不能修改信息'});
        }
      }, err => {
        this.$message({type: 'error', message: err.msg || ''})
      })
    ...
    getDictInfo(name,codeName){//根据数据字典查询
      let _this=this;
      if(_this[codeName].length){ return false; }
      _this.selectLoading = true;
      _this.$store.dispatch("findAllDrawerByName",name).then(    //查询执法领域
        res=>{
