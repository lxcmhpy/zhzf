  ...
  <div>
    <div>
      <div class="card-title">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <el-button v-if="params.type !== 'view'" type="primary" @click="addPunishment" style="margin-right:25px;float:right;" icon="el-icon-plus" size="cats-common">新增惩罚</el-button>
      </div>
      <el-table style="margin-left:25px;width:97%;margin-bottom:35px;" :data="tableData" resizable stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="awardTypeName" label="惩罚类型" :show-overflow-tooltip="true" align="center">
        </el-table-column>
        <el-table-column prop="awardDate" label="惩罚日期" :show-overflow-tooltip="true" align="center">
        </el-table-column>
        <el-table-column prop="awardDept" label="惩罚机构" :show-overflow-tooltip="true" align="center">
        </el-table-column>
        <el-table-column prop="reward" label="惩罚原因" :show-overflow-tooltip="true" align="center">
        </el-table-column>
      ...
      <div class="paginationBox" style="margin-top:-25px;margin-bottom:10px;text-align:center;">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" background :page-sizes="[10, 20, 30, 40, 50]" layout="prev, pager, next,sizes,jumper" :total="totalPage"></el-pagination>
      </div>
    </div>
    <!-- 新增、修改、查看页面 -->
    <addPunishmentComp ref="addPunishmentCompRef" :params="params" @getAllPunishment="getPunishment">
    </addPunishmentComp>
  </div>
</template>
  ...
  methods: {
    ...
    deletePunishment(row) {
      ...
      this.$confirm('确定要删除吗?', '提示', {
        ...
        .then(() => {
          _this.$store.dispatch('deleteAwardByIdMoudle', row.awardId).then(
            res => {
              if (res.code === 200) {
                _this.catsMessage({
                  type: 'success',
