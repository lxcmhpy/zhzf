  ...
  <div>
    <div>
      <div class="card-title">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <el-button v-if="params.type !== 'view'" type="primary" @click="addTrain" style="margin-right:25px;float:right;" icon="el-icon-plus" size="cats-common">新增培训</el-button>
      </div>
      <el-table style="margin-left:25px;width:97%;margin-bottom:35px;" :data="tableData" resizable stripe @selection-change="selectData">
  ...
  methods: {
    ...
    deleteTrain(row) {
      ...
      this.$confirm('确定要删除吗?', '提示', {
        ...
        .then(() => {
          _this.$store.dispatch('deleteTrainInfoByIdMoudle', row.trainId).then(
            res => {
              if (res.code === 200) {
                _this.catsMessage({
                  type: 'success',
