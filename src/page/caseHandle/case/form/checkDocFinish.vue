<template>
  <el-dialog :title="titleMap[dialogStatus]" custom-class="leftDialog" :visible.sync="visible" top="0" width="40%" :close-on-click-modal="false">
    <el-form :model="addUserForm" :rules="rules" ref="addUserForm" label-width="100px" class="addOrganClass" @close="closeDialog" :close-on-click-modal="false">
      <div class="part">
        ...
        <div class="item">
          <el-form-item label="执法机构" prop="organId">
            ...
            <el-select v-model="addUserForm.organId" placeholder="请选择执法机构" @change="getDepartment">
              <el-option v-for="item in getOrganList" :key="item.id" :label="item.name" :value="item.id"></el-option>
            </el-select>
          </el-form-item>
        </div>
        <div class="item">
