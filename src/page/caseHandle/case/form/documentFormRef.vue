  ...
  <div>
    ...
    >
      ...
      <div class="userList a">
        <el-checkbox-group v-model="checkedDocId" class="checkboxGroup">
          <el-tooltip
            ...
            placement="top-start"
          >
            <el-checkbox class="checkboxItem" :key="item.storageId" :label="item.storageId">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          :style="fatherCom==='inforCollection' ? 'top: -80px':null"
        ...
        <div style="height:auto;">
          ...
          <div v-show="showCover=='pdf'">
            <object>
              <embed
                class="print_info"
                style="padding:0px;width: 790px;margin:0 auto;height:1150px !important"
                name="plugin"
                id="plugin"
                :src="docSrc"
          ...
          <div style="position:absolute;bottom:150px;right: 20px;width:100px;">
            ...
            <el-button
              @click="showNext('next')"
              :disabled="nowShowPdfIndex == this.checkedDocId.length-1 ? true : false"
            >下一张</el-button>
          </div>
        </div>
      </div>
    ...
    getByMlCaseId() {
      ...
      findVoByDocCaseIdApi(this.caseId).then(
        (res) => {
          ...
          console.log("awdasdadad", _this.caseList);
        },
        (error) => {
          console.log(error);
        }
    ...
    showCover() {
      if (this.$route.name != "case_handle_archiveCover") {
        ...
        this.$router.push({
          ...
          params: { clickIsDoc: JSON.stringify(item) },
        });
        return;
      }
