    ...
    <div class="print_info" id="remindLetter_print">
      <el-form :rules="rules" ref="docForm" :inline-message="true" :inline="true" :model="docData">
        ...
        <p>
          ...
          <el-form-item prop="decidedTime" class="pdf_datapick" style="width:140px" :rules="fieldRules('decidedTime',propertyFeatures['decidedTime'])">
              <el-date-picker v-model="docData.decidedTime" type="date" format="yyyy年MM月dd日" placeholder="    年  月  日"  value-format="yyyy-MM-dd" :disabled="fieldDisabled(propertyFeatures['decidedTime'])">
              </el-date-picker>
          </el-form-item>
          对你（单位）作出了《代履行决定书》，决定书编号为
          <el-form-item prop="caseNumberCopy1" class="pdf_datapick" style="width:140px" :rules="fieldRules('caseNumberCopy1',propertyFeatures['caseNumberCopy1'])">
              <el-input  v-model="docData.caseNmuberCopy1"  :maxLength='maxLength' :disabled="fieldDisabled(propertyFeatures['caseNmuberCopy1'])"></el-input>            
          </el-form-item>，根据《中华人民共和国行政强制法》第五十一条的规定，
        </p>
        <p>
          现就有关事项催告如下，请你（单位）按要求履行：
        </p>
        <p>
          1.履行标的：
          <span>
