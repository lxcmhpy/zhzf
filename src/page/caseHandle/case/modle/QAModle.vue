<template>
  ...
  >
    ...
    >
      ...
      <div
        v-for="(domain, index) in addBannerForm.domains"
        :key="domain.key"
        class="askAndquestionCon"
      >
        <el-form-item
          :label="'问' + (index + 1)"
          :prop="'domains.' + index + '.question'"
          class="domainsBox"
        >
          <el-input
            type="textarea"
          ...
          <span class="handleBtn"
            ...
            ><i
              type="primary"
              v-show="index == addBannerForm.domains.length - 1"
...
export default {
  ...
  methods: {
    ...
    overFloeEdit() {
      ...
      this.$refs["addBannerForm"].validate((valid) => {
        if (valid) {
          ...
          _this.$emit(
            "QAModleInfo",
            JSON.stringify(_this.addBannerForm.domains)
          );
          _this.visible = false;
        }
      });
    ...
    findModelList(modelTypeId) {
      findModelListByModelTypeIdApi(modelTypeId).then(
        (res) => {
          console.log(res);
