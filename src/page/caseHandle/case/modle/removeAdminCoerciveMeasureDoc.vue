...
<template>
    <div class="print_box">
      <div class="print_info"  id="removeAdminCoercive-print">
        <el-form :rules="rules" ref="docForm" :inline-message="true" :inline="true" :model="docData">
          ...
          <p>
            依照《中华人民共和国行政强制法》第二十八条第一款第<span>
            ...
            <span>
              <el-form-item prop="removeDate" class="pdf_datapick">
                <el-date-picker v-model="docData.removeDate" type="date" format="yyyy年MM月dd日" placeholder="    年  月  日">
                </el-date-picker>
              </el-form-item>
            </span>起解除该行政强制措施。
          </p>
        ...
        <!-- 添加弹出框 -->
        <el-dialog title="退还财物清单" :visible.sync="addVisible" width="60%" v-loading="addLoading" :before-close="handleClose">
            <div>
              <div>
                <el-form ref="docForm">
          ...
          <!-- 悬浮按钮 -->
          <casePageFloatBtns :pageDomId="'removeAdminCoercive-print'" :formOrDocData="formOrDocData" @submitData="submitData" @saveData="saveData" @backHuanjie="submitData"></casePageFloatBtns>
  ...
  data() {
   ...
   methods: {
    ...
    addTableData(){
      ...
      if(length == 0){
        this.tableDatas.push({'evidenceNo': 1,'spec' : '1', 'amount' : 1});
      }else{
        this.tableDatas.push({'evidenceNo': Number(this.tableDatas[length - 1].evidenceNo) + 1,'spec' : '1', 'amount' : 1});
      }
    },
    ...
    submitForm(formName){
      for(let i=0; i<this.tableDatas.length; i++){
          if(!this.tableDatas[i].returnItemsName && !this.tableDatas[i].spec && !this.tableDatas[i].amount && !this.tableDatas[i].remark){
            this.$message({
              message: '数据至少有一项不为空！',
              type: 'warning'
            });
