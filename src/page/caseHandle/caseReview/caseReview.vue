  ...
  <div class="com_searchAndpageBoxPadding">
    <div :class="hideSomeSearch ? 'searchAndpageBox' : 'searchAndpageBox searchAndpageBox2'">
      <div class="handlePart caseHandleSearchPart">
        <div class="item">
          <el-button type="primary" size="medium" icon="el-icon-plus" @click="caseCaseReview">添加调阅案件</el-button>
        </div>
        <div>
          <el-form
        ...
        <el-table :data="tableData" stripe style="width: 100%" highlight-current-row height="100%">
          ...
          <el-table-column prop="consultStatus" label="案卷调阅状态" align="center" min-width="100">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.consultStatus === 3">
                <el-button type="primary" size="small" @click="reviewCase(scope.row)">调阅中</el-button>
              </span>
...
import { getCaseReviewList } from "@/api/caseHandle";
  ...
  methods: {
    ...
    getCaseReviewList() {
      const queryData = Object.assign(this.caseSearchForm, {
        ...
        size: this.pageSize,
        partyName: this.caseSearchForm.party
      });
      getCaseReviewList(queryData).then(
        (res) => {
          if (res.code === 200) {
