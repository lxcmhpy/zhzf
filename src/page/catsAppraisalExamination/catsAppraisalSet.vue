     ...
     <div class="cats-layout-page-content">
         <div class="searchAndpageBox toggleBox">
             <div class="handlePart" style="margin-left: 0px;">
                 <el-button type="primary" size="cats-common" @click="addPykh">
             ...
             <div class="tablePart">
                 ...
                 <el-table :data="tableData" stripe resizable border style="width: 100%;height:100%;"
                     @expand-change="load" row-key="id" :expand-row-keys="expandList" current-row-key="id">
                         ...
                         <template slot-scope="scope">
                             <div style="padding:0px 0px 35px 45px;min-width:620px;min-height: 550px;">
                                 <div class="handlePart" style="margin: 8px 0px 8px 0px;">
                                     <el-button type="primary" size="cats-common" @click="addZbConfig(scope.row)">
                                         <i class="iconfont law-submit-o f12"></i> 添加二级考核指标
                                     </el-button>
                                 </div>
                                 <div style="border: 1px solid #f4f4f4;" v-if="zbList.length>0">
                                     <el-table class="xzList" :data="zbList" stripe resizable border
                                         style="height:100%;" @expand-change="loadXzList" row-key="id"
                                         :expand-row-keys="expandXzList" current-row-key="id">
                                         <el-table-column type="expand" width="45">
                                             <!-- <template slot="header" >
                                            考评细则
                                        </template> -->
                                             <template slot-scope="scope1">
                                                 <div
                                                     style="padding:0px 0px 35px 45px;min-width:620px;min-height: 550px;">
                                                     <div class="handlePart" style="margin: 8px 0px 8px 0px;">
                                                         <el-button type="primary" size="cats-common"
                                                             @click="addXzConfig(scope1.row)">
                                                             <i class="iconfont law-submit-o f12"></i> 添加考评细则
                                                         </el-button>
                                                     </div>
                                                     <div style="border: 1px solid #f4f4f4;" v-if="xzList.length>0">
                                                         <el-table class="xzList" :data="xzList" stripe resizable border
                                                             style="height:100%;">
                                                             <!-- <el-table-column prop="reviewType" label="评查类别" align="center" ></el-table-column> -->
                                                             <el-table-column prop="nrxm" label="考评内容及评分标准"
                                                             ...
                                                             <el-table-column label="操作" align="center" width="150">
                                                                 <template slot-scope="scope2">
                                                                     <el-button type="text" @click.stop
                                                                         @click="updateXzConfig(scope2.$index, scope2.row)">
                                                                         修改</el-button>
                                                                     <el-button type="text" @click.stop
                                                                         @click="deleteDetailZpById(scope1.row,scope2.row)">
                                                                         删除</el-button>
                                                         ...
                                                         <div class="paginationBox">
                                                             <div v-if="total2/xzForm.size > 1">
                                                                 <el-pagination
                                                                     ...
                                                                     :current-page="xzForm.current" background
                                         ...
                                         <el-table-column prop="indexTwo" label="二级考核指标" align="center">
                                         </el-table-column>
                                         <!-- <el-table-column prop="assessType" label="考核类别" align="center" width="100"></el-table-column> -->
                                         <el-table-column label="操作" align="center" width="150">
                                             <template slot-scope="scope1">
                                                 <el-button type="text" @click.stop
                                                     @click="updateZbConfig(scope1.$index, scope1.row)">修改</el-button>
                                                 <el-button type="text" @click.stop
                                                     @click="deletePykhMetricsById(scope.row,scope1.row)">删除</el-button>
                                     ...
                                     <div class="paginationBox">
                                         <div v-if="total1/zbForm.size > 1">
                                             <el-pagination @size-change="(size)=>handleZbSizeChange(scope.row.id,size)"
                                                 @current-change="(current)=>handleZbCurrentChange(scope.row.id,current)"
                                                 :current-page="zbForm.current" background
                                                 :page-sizes="[5, 10, 20, 30, 40]" :page-size="zbForm.size"
                     ...
                     <el-table-column label="操作" align="center" width="150">
                         ...
                         <template slot-scope="scope">
                             <el-button type="text" @click.stop @click="updatePykhConfig(scope.$index, scope.row)">修改
                             </el-button>
             ...
             <div class="paginationBox">
                 <div v-if="total/form.size > 1">
                     <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                         :current-page="form.current" background :page-sizes="[5, 10, 20, 30, 40]"
                         :page-size="form.size" layout="prev, pager, next,sizes,jumper" :total="total"></el-pagination>
                 </div>
                 <div class="noMore" v-else>没有更多了</div>
             </div>
         </div>
         <el-drawer title="考核要求" modal-append-to-body :before-close="(done)=>resetClose('pykhObj',done)" direction="rtl"
             ...
             <div class="demo-drawer__footer" style="text-align:center">
                 <el-button type="primary" @click="addPykhConfig" :loading="loading">{{ loading ? '提交中 ...' : '确 定' }}
                 </el-button>
         ...
         <el-drawer title="考核指标" modal-append-to-body :before-close="(done)=>resetClose('zbObj',done)" direction="rtl"
                     ...
                     <div class="item">
                         <el-form-item label="二级考核指标" prop="indexTwo">
                             <el-input type="textarea" :autosize="{ minRows: 5, maxRows: 10}" v-model="zbObj.indexTwo"
                                 placeholder="请输入二级考核指标"></el-input>
                         </el-form-item>
                     </div>
                     <!-- <div class="item">
             ...
             <br>
             <div class="demo-drawer__footer" style="text-align:center">
                 <el-button type="primary" @click="addOrUpdatePykhMetrics" :loading="loading">
                     {{ loading ? '提交中 ...' : '确 定' }}</el-button>
                 <el-button @click="closeDraw1">取 消</el-button>
             </div>
         </el-drawer>
         <el-drawer title="考核细则" modal-append-to-body :before-close="(done)=>resetClose('xzObj',done)" direction="rtl"
             ...
             <div style="padding:0px 0px 35px 45px;min-width:620px;">
                 <el-form :model="xzObj" ref="xzObj" :rules="rules2" class="checkSearchForm" label-width="150px">
                        ...
                        <el-form-item label="评查类别" prop="indexTwo">
                         ...
                         <div class="item">
                             <el-form-item label="考评内容及评分标准" prop="nrxm">
                                 <el-input :autosize="{ minRows: 5, maxRows: 10}" type="textarea" v-model="xzObj.nrxm"
                                     placeholder="请输入考评内容及评分标准"></el-input>
                             </el-form-item>
                         </div>
                         <!-- <div class="item">
                     ...
                     <div class="demo-drawer__footer" style="text-align:center">
                         <el-button type="primary" @click="addorUpdateDetailZp" :loading="loading">
                             {{ loading ? '提交中 ...' : '确 定' }}</el-button>
 ...
 <script>
     import {
         findPykhConfigByPage,
         addOrUpdatePykhConfig,
         deletePykhConfigById,
         findPykhMetricsByPage,
         addOrUpdatePykhMetrics,
         deletePykhMetricsById,
         findPykhZpByPage,
         addorUpdateDetailZp,
         deleteDetailZpById,
         findListVoByPykhZp
         ...
         methods: {
             ...
             async addPykh() {
                 ...
                 this.$set(this, 'pykhObj', {
                     ...
                     organName: "",
                     organId: "",
                     oneType: this.oneZblxList.data[0].name,
                     oneTypeId: this.oneZblxList.data[0].id
                 });
             ...
             resetClose(formName, func) {
                 this.$refs[formName].resetFields();
                 func();
             },
             async updatePykhConfig(index, row) {
                 this.oneZblxList = await this.$store.dispatch("getDictListDetailTb", PYKH_DATA.one_zblx);
                 this.pykhObj = JSON.parse(JSON.stringify(row));
                 this.drawer = true;
             ...
             async updateZbConfig(index, row) {
                 // this.zbObj = row;
                 // this.khlbList = await this.$store.dispatch("getDictListDetailTb", PYKH_DATA.khlb);
                 this.$set(this, 'zbObj', JSON.parse(JSON.stringify(row)));
                 this.drawer1 = true;
             ...
             async updateXzConfig(index, row) {
                 // this.zbObj = row;
                 // this.khlbList = await this.$store.dispatch("getDictListDetailTb", PYKH_DATA.khlb);
                 this.$set(this, 'xzObj', JSON.parse(JSON.stringify(row)));
                 this.drawer2 = true;
                             ...
                             addOrUpdatePykhConfig(_this.pykhObj).then(
                                 res => {
                                     if (this.updateIndex === null) {
                                         ...
                                         // 更新
                                         _this.drawer = false;
                                         _this.$set(_this.tableData, _this.updateIndex, _this
                                             .pykhObj);
                                     }
                                     _this.$set(_this, 'pykhObj', {
                                         ...
                                         organId: "",
                                         oneType: "",
                                         oneTypeId: _this.oneZblxList.data[0].id
                                     });
                                     _this.closeLoading();
                                     _this.catsMessage({
                                         type: 'success',
