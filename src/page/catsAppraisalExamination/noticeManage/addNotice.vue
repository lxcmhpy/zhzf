<template>
  <el-dialog :title="dialogTitle" :visible.sync="visible" @close="closeDialog" :close-on-click-modal="false" width="1100px">
    <el-form :model="addNoticeForm" :rules="rules" ref="addNoticeForm" label-width="80px">
      ...
      <div class="item" :hidden="showUpload">
        <el-form-item label="公告内容" prop="content">
          <quill-editor v-model="addNoticeForm.content" ref="myQuillEditor" :options="editorOption" @blur="onEditorBlur($event)" @focus="onEditorFocus($event)" @change="onEditorChange($event)">
          </quill-editor>
        </el-form-item>
      </div>
      <div class="item" :hidden="!showUpload">
        <el-upload class="upload-demo" accept=".pdf" ref="caseUpload" :http-request="saveFile" action="https://jsonplaceholder.typicode.com/posts/" multiple :file-list="fileList" :limit="1">
  ...
  methods: {
    ...
    showModal(type, data) {
      ...
      if (type == 0) {
        ...
        ;(this.addNoticeForm.title = data.title), (this.addNoticeForm.noticeType = data.noticeType)
