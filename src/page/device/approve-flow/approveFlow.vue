  ...
  <div class="fullBox departBox">
    ...
    <div class="departTable">
      ...
      <el-table
        ...
        height="70%"
      >
        <el-table-column prop="organId" label=" " align="center" v-if="false"></el-table-column>
...
import { findOrganTreeByCurrUser,queryApproveProcessPage,initAllApprove } from "@/api/device/approveFlow.js";
import { getAllOrigin } from "@/api/device/param.js";
export default {
  watch: {
    filterText(val) {
  ...
  methods: {
    ...
    doaApploy(row){
      ...
      this.paramList=[]
      let data = [];
        data.push({"id":row.organId,"organName":row.organName,"index":0});
       this.$refs.approveDeployRef.showModal(data,row.organId);
    },
    //获取选中的机构下的机构
    getSelectOrgan() {
      ...
      queryApproveProcessPage(data).then(
        res => {
