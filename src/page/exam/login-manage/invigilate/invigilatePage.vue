    ...
    <div class="rightPart">
      <div class="handlePart">
        <el-form :inline="true" ref="userForm" label-width="95px" rules="userFormRef">
          <el-row>
            <el-form-item label="姓名或身份证号" prop="personName">
              <el-input v-model="userForm.personName"></el-input>
            </el-form-item>
            <el-form-item label=" " label-width="13px">
              <el-button  title="搜索" class="commonBtn searchBtn" size="medium" icon="iconfont law-sousuo" @click="getExamPersonsSearchInfo"></el-button>
              <el-button
                title="重置" class="commonBtn searchBtn" size="medium" icon="iconfont law-zhongzhi"></el-button>
            </el-form-item>
            <P>
              <el-form-item label=" " label-width="13px" style="float:left">
                <el-button type="primary" icon="el-icon-time" size="medium" @click="setBatchExamDelayedInfo">批量延迟</el-button>
              </el-form-item>
              <el-form-item label=" " label-width="13px" style="float:left">
      ...
      <div class="tablePart">
        ...
        >
          ...
          <el-table-column prop="examPerId" label="examPerId" v-if="false"></el-table-column>
          ...
          <el-table-column label="操作项" width="330">
            <template slot-scope="scope">
              <div>
                <el-button type="text" @click="exanRecord('1',scope.row)">考场记录</el-button>
                <el-button type="text" @click="exanRecord('2',scope.row)">强制收卷</el-button>
                <el-button type="text" @click="exanRecord('3',scope.row)">延迟收卷</el-button>
                <el-button type="text" @click="exanRecord('4',scope.row)">登录重置</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  data() {
    return {
      userForm:{
        personName:'',
        examId:'',//考试信息从上一个页面传过来
        roomId:'',//考场信息从上一个页面传过来
        examStatue:'',//考试状态默认值用于页面查询过滤：0：未开始   1：考试中 2：考试结束
      },
      examMsg: {
        //考试信息
        startTime: "2020.3.4",
  ...
  methods: {
    //搜索查询条件
    getExamPersonsSearchInfo(){
      this.$store.dispatch("getExamPersonsSearchInfo",data).then(
        res=>{
          if(res.code===200){
    ...
    //批量启用getExamPersonsSearchInfoApi,Api
    setBatchExamDelayedInfo(){
      this.$store.dispatch("setBatchExamDelayedInfo",this.userForm).then(
        res=>{
          if(res.code===200){
