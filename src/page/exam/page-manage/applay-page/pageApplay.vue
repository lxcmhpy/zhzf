    ...
    <div class="com_searchAndpageBoxPadding">
        <div class="searchPage toggleBox">
            <div class="handlePart">
                <el-form :inline="true" :model="maintainPageForm" label-width="120px;"  ref="maintainPageFormRef">
                    <div>
                        <div class="item">
                            <el-row>
                                ...
                                <el-form-item label="试卷类型" prop="pageType" style="margin-left:20px;">
                                    <el-select v-model="maintainPageForm.pageType" placeholder="试卷类型" remote  @focus="getDictInfo('人员信息-试卷类型','pageTypeList')">
                                        <el-option
                                            v-for="value in pageTypeList" :key="value.id" :label="value.name" :value="value.id">
                                        </el-option>
                                    </el-select>
                                </el-form-item>
                                <el-form-item style="margin-left:20px;">
                                    <el-button title="搜索" class="commonBtn searchBtn" size="medium" icon="iconfont law-sousuo"   @click="getPageInfoList"></el-button>
...
export default {
    ...
    data(){
        return{
            tableData:[{'pageId':'123','pageName':"交通部某某考试",'pageType':'模拟试卷','score':100,'createId':'yyq','oid':'交通部','verifyStatus':'审核通过'}],
            pageTypeList:[],
            maintainPageForm:{
                pageName:"",
                pageType:"",
    ...
    methods:{
        ...
        //删除试卷
        deletePageInfo(row){
            console.info(JSON.stringify(row.pageId))
            let _this = this 
                _this.deleteById("deletePageManage",row.pageId);
                _this.getPageList();//重新加载页面数据
        },
        // 日志重置
        resetLog() {
