    ...
    submitEntry(){
      this.$refs.entryForm.validate((valid) => {
        if (valid) {
          ...
          // 提交操作
          this.$store.dispatch('invigilatorSubmitInfo', this.entryForm).then(res => {
            // setToken(res.data);
            setToken("TokenKey", res.data, 110);
            this.$router.push({ path: '/invigilator' });
          }, err => {
            console.log(err);
          })
          loading.close();
...
.page-contain{
  ...
  .exam-info{
    position: absolute;
    background: url('../../../../static/images/img/exam/card_left.png') no-repeat;
