        ...
        <el-row>
          <el-col :span="8">
            <div class="examinee-info">
              ...
              <p v-if="examineeData.personInfo && examineeData.personInfo.personName" class="name">{{ examineeData.personInfo.personName }}</p>
              <p v-if="examineeData.personInfo && examineeData.personInfo.oname" class="componey">{{ examineeData.personInfo.oname }}</p>
            </div>
          </el-col>
          <el-col :span="16">
            <div class="exam-info">
              <div class="time-info">
                ...
                <span class="time-prompt">{{ timeInfo.year }}年{{ timeInfo.month }}月{{ timeInfo.day }}日</span>
              ...
              <div class="exam-list">
                ...
                <div
                  ...
                  <div class="exam-status">
                    <el-button v-if="exam.examStatus == '1'" type="primary" class="entry-btn" @click="entryExam(exam)">进入</el-button>
...
export default {
  ...
  methods: {
    // 获取考生信息
    getExamineeInfo(){
      this.$store.dispatch('getJoinExamPerson', this.examineeName).then(res => {
        if(res.code === 200){
      ...
      if(examList && examList.length){
        ...
        examList.forEach(item => {
          const status = new Promise((resolve, reject) => {
            this.$store.dispatch('getPersonExamStatus', { examId: item.examId, loginName: this.examineeName }).then(res => {
              if(res.code === 200){
                resolve(res.data)
              }else{
                reject(false);
    ...
    startAnswer(){
      ...
      sessionStorage.setItem('ExamUserInfo', JSON.stringify({ personInfo: this.examineeData.personInfo, examInfo: this.selectedExam }));
...
.count-contain{
  ...
  overflow: hidden;
  min-height: 400px;
  background: url('../../../../static/images/img/exam/count_down_bottom.png') no-repeat;
