<template>
  <div>
    ...
    <el-drawer
      :title="drawerTitle"
      :visible.sync="newModleTable"
      direction="rtl"
      :wrapperClosable="false"
      size="50%"
      :before-close="handleDrawerClose"
      class="dialo dialog_unlaw max-group-prepend card-drawer"
    >
        ...
        >
          ...
          <el-col :span="17">
            <el-form-item label="模板标题" prop="title">
              <el-input
                ...
                placeholder="请输入模板标题"
                clearable
                @input="handleChangeTemplateTitle"
              >
              </el-input>
            </el-form-item>
          </el-col>
          <p class="border-title card-title-margin clear">
            添加指标
            <span class="card-add-btn">
              <el-button icon="el-icon-plus" size="medium" @click="addGroup()"
                >添加新字段组</el-button
              >
            </span>
          </p>
          ...
          <div class="collapse-title-foem">
            <el-collapse v-model="activeNames" class="clear">
              <div
                v-for="(item, index) in formData.templateFieldList"
                :key="index"
              >
                <el-collapse-item :name="item.sort" style="margin-top: 12px">
                  <template slot="title">
                    ...
                    >
                      <el-select
                        ...
                        allow-create
                        clearable
                        placeholder="请输入字段组名称，可为空"
                        @change="changeGroup(item)"
                      >
                        <el-option
                          v-for="(commonField, index) in commonGroupFieldList"
                          :key="index"
                          :label="commonField.classs"
                          :value="commonField.classs"
                          :disabled="disabledChooseGruop(commonField)"
                        ></el-option>
                      </el-select>
                    </el-form-item>
                    <i
                      ...
                      @click="delField(item, formData.templateFieldList)"
                    ></i>
                  </template>
                  <!-- 是否转立案字段 -->
                        ...
                        <el-col :span="22">
                          ...
                          >
                            <el-select
                              ...
                              :disabled="field.status === 0 ? true : false"
                            >
                              <el-option
                                label="文本型"
                                value="文本型"
                  ...
                  <el-form
                    v-else
                    ref="childForm"
                    v-for="(field, index1) in formData.templateFieldList[index]
                      .fieldList"
                    :key="index1"
                    :model="formData.templateFieldList[index]"
                  >
                    <el-row :gutter="20">
                      <el-col :span="2">
                        <el-form-item label-width="0" prop="required">
                      ...
                      <el-col :span="16">
                        ...
                        >
                          ...
                          <el-select
                            ...
                            clearable
                            placeholder="请填写字段名称"
                            @change="changeField(field.info, field)"
                            ref="test"
                            :id="field.info ? field.info.field : ''"
                          >
                            <el-option
                              v-for="(commonField, index3) in commonFieldList"
                              :key="index3"
                              :label="commonField.title"
                              :value="commonField"
                              :disabled="disabledChoose(commonField)"
                            ></el-option>
                          </el-select>
                        </el-form-item>
                      </el-col>
                      <el-col :span="6">
                        ...
                        <i
                          ...
                          @click="delField(field, item.fieldList)"
                          style="
                            margin-left: 18px;
                            margin-top: -38px;
                            float: right;
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <i
                            v-if="field.status === 0 ? false : true"
                            class="el-icon-remove-outline"
                            style="margin-right: 14px"
                            @click="delField(radio, field.options)"
                          ></i>
                          <el-input
                            size="mini"
                            v-model="radio.value"
                          ...
                          <i
                            ...
                            @click="addRadioList(field.options)"
                          ></i>
                        </el-form-item>
                      </el-col>
                    </el-row>
                      ...
                      >
                        <el-row>
                          ...
                          >
                            <el-row>
                              <el-col :span="12">
                                <el-radio label="yyyy-MM-dd HH:mm:ss"
                                  >精准时间（2020-03-11 12:12:12）</el-radio
                                >
                              </el-col>
                              <el-col :span="12">
                                <el-radio label="yyyy-MM-dd HH:mm"
                                  >日期和时间（2020-03-11 12:12）</el-radio
                                >
                              </el-col>
                              <el-col :span="12">
                                <el-radio label="yyyy-MM-dd"
                                  >仅日期（2020-03-11 ）</el-radio
                                >
                              </el-col>
                              <el-col :span="12">
                                <el-radio label="HH:mm"
                                  >仅时间（09:12）</el-radio
                                >
                              </el-col>
                              <!-- <el-col :span="12">
          ...
          >
            ...
            >
              <el-radio
                ...
                value="是"
                style="width: 5%"
                @click.native.prevent="clickitem('是')"
              ></el-radio>
              <el-button
                type="primary"
              ...
              <el-radio
                label="否"
                value="否"
                @click.native.prevent="clickitem('否')"
              ></el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item
            label="相关记录"
            class="modle-radio chose-mine"
            prop="releventRecords"
          >
            <el-radio-group v-model="formData.releventRecords">
              <el-radio
                label="当事人"
                value="当事人"
                @click.native.prevent="clickitem2('当事人')"
              ></el-radio>
              <el-radio
                label="车辆"
                value="车辆"
                @click.native.prevent="clickitem2('车辆')"
              ></el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item
          ...
          >
            <el-radio-group v-model="formData.operateRecords">
              <el-radio
                label="是"
                value="是"
                @click.native.prevent="clickitem3('是')"
              ></el-radio>
              <el-radio
                label="否"
                value="否"
                @click.native.prevent="clickitem3('否')"
              ></el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item
          ...
          <el-form-item label="模板图标">
            ...
            >
              <li
                ...
                :key="icon.icon"
                class="record-icon-box"
                @click="changIcon(icon)"
              >
                <div class="record-icon-box-content">
                  <img
                    :src="'./static/images/img/record/' + icon.icon + '.png'"
                    alt=""
                  />
                </div>
              </li>
              <li slot="reference" class="record-icon-box">
                <div class="record-icon-box-content" style="line-height: 90px">
                  <img
                    v-if="formData.icon != '' && formData.icon"
                    :src="
                      './static/images/img/record/' + formData.icon + '.png'
                    "
                    alt=""
                  />
                  <span
          ...
          <el-form-item label="适用范围" prop="scopeOfUse">
            ...
            >
              <div class="el-form-item__content">
                ...
                <el-form-item
                  v-if="formData.scopeOfUse == '指定人员使用'"
                  class="lawPersonBox card-user-box"
                  :prop="
                    formData.scopeOfUse == '指定人员使用'
                      ? 'templateUserIdList'
                      : 'pacholor'
                  "
                >
                  <el-select
                    ref="templateUserIdList"
                    value-key="userId"
                    v-model="formData.templateUserIdList"
                    multiple
                    filterable
                  ...
                  >
                    <span
                      ...
                      style="background: #eaedf4; height: 34px; display: block"
                      >本机构执法人员({{ LawOfficerList.length }})</span
                    >
                    <el-option
                      v-for="item in LawOfficerList"
                      :key="item.id"
              ...
              <div class="el-form-item__content">
                ...
                <el-form-item
                  v-if="formData.scopeOfUse == '机构内使用'"
                  class="lawPersonBox card-user-box organClass"
                  :prop="
                    formData.scopeOfUse == '机构内使用'
                      ? 'templateOrgan'
                      : 'pacholor'
                  "
                >
                  ...
                  >
                        ...
                        >
                          ...
                          >
                            <span>
                              ...
                              ></i>
                              <span
                                :class="data.children ? '' : 'hasMarginLeft'"
                                >{{ node.label }}</span
                              >
                            </span>
                          </span>
                    ...
                    <el-input
                      slot="reference"
                      v-model="formData.templateOrgan"
                      placeholder="请选择机构"
                      :disabled="true"
                      style="width: 100%"
                    >
          ...
          <el-form-item label="模板管理者">
            <el-form-item
              class="lawPersonBox card-user-box-big"
              style="width: 100%"
            >
              <el-select
                ref="templateAdminIdList"
                value-key="userId"
                v-model="formData.templateAdminIdList"
                multiple
                filterable
                @remove-tag="removeAdmintag"
                @change="changeAdmin"
              >
                <span
                  class="el-select-dropdown__item"
      ...
      <div class="demo-drawer__footer footer_fixed">
        <el-button type="primary" @click="handleSubmitForm('formData')"
          >发布</el-button
        >
...
import {
  saveOrUpdateRecordModleApi,
  findCommonGroupFieldApi,
  findAllCommonGroupFieldApi,
  findRecordModleByIdApi,
  findRecordlModleFieldByIdeApi,
  findAllCommonFieldApi,
  findAllCandidateFieldApi,
  getDocumentNameList,
} from "@/api/inspection";
  ...
  computed: {
    ...
    disabledChooseGruop(item) {
      return function (item) {
        ...
        let findItemIndex = valueList.findIndex(
          (item2) => item2 == item.classs
        );
          ...
          findRecordModleByIdApi(this.editId).then(
            (res) => {
              if (res.code == 200) {
                ...
                if (
                  _this.formData.templateUser &&
                  _this.formData.templateUserId
                ) {
                  let user = _this.formData.templateUser.split(",");
                  let userId = _this.formData.templateUserId.split(",");
                  user.forEach((element, index) => {
                    _this.formData.templateUserIdList.push({
                      userId: userId[index],
                ...
                let admin = _this.formData.templateAdmin
                  ? _this.formData.templateAdmin.split(",")
                  : [];
...
debugger
    ...
    addGroup() {
      ...
      this.formData.templateFieldList.push({
        sort: this.globalContGroup,
        classs: "",
        fieldList: [],
        classId: "",
      });
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (canSubmit) {
            ...
            let defaultFieldList = JSON.stringify(
              this.defaultTemplateFieldList
            );
            if (fieldList == defaultFieldList) {
              ...
              this.$confirm("完成当前模板，并发布？", "模板发布", {
                ...
                data.templateAdminIdList.forEach((element) => {
                  // 使用userID
                  data.templateAdminId =
                    data.templateAdminId + "," + element.userId;
                  data.templateAdmin =
                ...
                data.templateUserIdList.forEach((element) => {
                  ...
                  // 使用userID
                  data.templateUserId =
                    data.templateUserId + "," + element.userId;
                });
                // 未做ie浏览器兼容处理
    ...
    changeDisabledStatus(field) {
      ...
      // 判断之前有没有选过
      this.$nextTick(() => {
        console.log(document.getElementsByName("filedNameFlag")[0].id);
    ...
    noticeMsg(data) {
      if (this.formData.releventRecords == "当事人" && !this.partyFlag) {
        ...
        if (
          this.formData.releventRecords == "车辆" &&
          !this.vehicleShipIdFlag
        ) {
          // 车辆字段没有
