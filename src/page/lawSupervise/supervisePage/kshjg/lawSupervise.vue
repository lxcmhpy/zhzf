<template>
  ...
  <div id="lawSupervise"
       ref="lawSupervise"
       class="mainBox"
       :class="{'lawScreenFull':lawScreenFull}">
    <el-carousel :interval="8000"
                 indicator-position="none"
                 height="28px"
                 v-if="gjclList"
                 style="position:absolute;top:0px;line-height:28px;width:100%;font-size:12px;color:#20232B">
      <el-carousel-item v-for="(row,index) in gjclList"
                        :key="index.toString()">
    ...
    <div class="amap-page-container">
      ...
      <el-amap vid="lawSupervise"
        ...
        <el-amap-marker v-for="(marker,index) in markers"
                        ...
                        :template="marker.template"></el-amap-marker>
        <el-amap-info-window v-if="curWindow"
          ...
          <div :class="'lawWindowStyle'+curWindow.category">
            ...
            <div v-if="curWindow.category == 0">
              ...
              <externalVideoBtns :doing="videoDoing"
                                 @updateMakePhoneStatus="updateMakePhoneStatus"
                                 :curWindow="curWindow"></externalVideoBtns>
            ...
            <div v-else-if="curWindow.category == 1">
              <div class="lawWindowTitle">
                ...
                <div class="title"
                     style="color: #fff;font-weight: 200;">{{curWindow.other.name}}</div>
                <!-- <span></span> -->
              ...
              <!-- <div class="btns">
                ...
                <span class="phoneBtn greenBg"
                      @click="handleNodeClick(it)"
                      :class="{'lineh': it.nickName.length>5}"
                      v-for="(it,j) in curWindow.other.users"
                      :key="'s'+j">{{it.nickName}}</span>
              </div> -->
              <div class="btns"
                   v-show="!showMore">
                <div class="flex-title"><img :src="'./static/images/img/lawSupervise/icon_duiwu.png'">人员在线情况</div>
                <span class="phoneBtn greenBg"
                      @click="handleNodeClick(it)"
                      :class="{'lineh': it.nickName.length>5}"
                      v-for="(it,j) in curWindow.other.users.slice(0,10)"
                      :key="'s'+j">{{it.nickName}}</span>
                <span class="phoneBtn blueBg "
                      title="查看全部"
                      v-if="curWindow.other.users.length>10"
                      @click="showMore = true">···</span>
              </div>
              <div class="btns"
                   v-show="showMore">
                <div class="flex-title"><img :src="'./static/images/img/lawSupervise/icon_duiwu.png'">人员在线情况</div>
            ...
            <div v-else-if="curWindow.category == 2">
              <div class="lawWindowTitle">
                ...
                <div class="title"
                     style="color: #fff;font-weight: 200;">{{curWindow.other.vehicleNumber}}<span class="right"
                        style="margin-top:0px;">在线</span></div>
            ...
            <div v-else-if="curWindow.category == 3">
              ...
              <div class="lawWindowTitle">
                <i class="iconfont law-ship"></i>
                <div class="title"
                     style="color: #fff;font-weight: 200;">{{curWindow.other.shipNumber}}<span class="right"
                        style="margin-top:0px;">在线</span></div>
                <span></span>
                <!-- <div class="right">{{curWindow.other.enforceNo}}</div> -->
              </div>
            ...
            <div v-else-if="curWindow.category == 4">
              <div>
                <el-carousel ref="carousel"
                  ...
                  <el-carousel-item v-for="(item,index) in curWindow.other.imageList"
                ...
                <div class="con">
                  ...
                  <div>
                    <p>
                      ...
                      <span class="right"
                            @click="routerXsByName(curWindow.other.name)">详情</span>
                    </p>
                  </div>
                </div>
              </div>
              <div class="con ">
                <el-table style="width: 100%;line-height: 40px;"
          ...
          <div :class="'lawWindowStyle-po'"
            ...
            <div v-else-if="curWindow.category == 1">
              <div class="lawWindowTitle">
                <i class="iconfont law-zfj"></i>
                <div class="title"
                     :title="curWindow.other.name">{{curWindow.other.name}}</div>
                <span></span>
              </div>
            </div>
            <!-- 2执法车辆 -->
            ...
            <div v-else-if="curWindow.category == 5">
              <div class="lawWindowTitle">
                <i class="iconfont law-zfj"></i>
                <div class="title"
                     :title="curWindow.other.nickName">{{curWindow.other.nickName}}</div>
              </div>
            </div>
            <!-- 6监管车辆 -->
            <div v-else-if="curWindow.category == 6">
      ...
      <div class="amap-position amap-rtl-box">
        <div class="amap-tool"
             style="position:relative;z-index:5;left:-120px;top:45px;"
             :class="{'left-500':drawer}">
          <el-popover placement="bottom-start"
                      trigger="click"
                      @hide="hidePopover">
            <div class="drop-down-menu">
              <el-cascader ref="areaCascader"
                           :options="areaList"
                ...
                <li>
                  <span>
                    <el-checkbox v-model="checked"
                                 @change="isCheckAll">{{radioVal}}</el-checkbox>&nbsp;
                  </span>
                  <!-- <el-radio v-model="radioVal" label="2">取消全选</el-radio> -->
                </li>
                <li v-for="subItem in tabList[0].children"
                    :key="subItem.name"
          ...
          <el-button size="medium"
                     ...
                     @click="lawScreenFull=!lawScreenFull">
          ...
          <el-popover placement="bottom-start"
            ...
            <div class="drop-down-menu">
              <ul>
                <li v-for="(item,index) in styleListNumber"
                    :class="{'select':styleIndexNumher==index}"
                    :key="index"
                    @click="styleIndexNumher=index">
                  <p>{{item}}</p>
                </li>
              </ul>
            </div>
          ...
          <div class="amap-main-content"
            ...
            <div class="echarts-box">
              <div class="title"
                ...
                近三个月执行情况
                <i v-if="status1 == true"
                   class="iconfont law-youyou right"></i>
                <i v-if="status1 == false"
                   class="iconfont law-zuozuo right"></i>
              </div>
              <el-collapse-transition>
                <div v-show="status1"
                     id="echartsBox1"
                     class="amap-chart"
                     style="height:200px;width:380px"></div>
              </el-collapse-transition>
            </div>

            <div class="echarts-box">
              <div class="title"
                   @click="status2 = !status2">
                近三个月查处排行
                <i v-if="status2 == true"
                   class="iconfont law-youyou right"></i>
                <i v-if="status2 == false"
                   class="iconfont law-zuozuo right"></i>
              </div>
              <el-collapse-transition>
            ...
            <div class="echarts-box">
              <div class="title"
                   @click="status3 = !status3">
                车辆预警
                <i v-if="status3 == true"
                   class="iconfont law-youyou right"></i>
                <i v-if="status3 == false"
                   class="iconfont law-zuozuo right"></i>
              </div>
              <el-collapse-transition>
                <div v-show="status3"
              ...
              <el-collapse-transition>
                <div v-show="status4">
                  <ul class="addHoverBg"
                    ...
                    <li v-for="(row,index) in gjclList"
                        :key="index"
                        @click="positionEvent1(row)">
                          ...
                          <div class="leftTabelHoverDiv"
                            ...
                            <div class="lawHoverTitle">
                              ...
                              <div class="gj-con">
                                ...
                                <span>{{row.vehicleNumber}}</span><span class="bgCgray"
                                      style="float:right">{{row&&row.checkTime?row.checkTime.split(' ')[1]:''}}</span>
                                <p><span class="bgCgray">站点：</span>{{row.siteName}}</p>
                              </div>
                            </div>
                          </div>
              ...
              <el-collapse-transition>
                <div v-show="status5">
                  <ul style="width: 100%;height: auto;"
                      slot="reference">
                    <li v-for="(row,index) in zfdList"
                        :key="index"
                        @click="positionEvent(row, 4)">
                      <div class="leftTabelHoverDiv"
                           style="padding: 0px;">
                        <div class="lawHoverTitle">
                          <div class="cxl">
                            <span class="f18 redC"
                                  :class="{'greenC': index ===0}">{{index === 0? '正常': '异常'}}</span><br>
      ...
      <!-- 底部浮动栏 -->
      <div class="amap-position"
           :class="'amap-' + direction1 + '-box'"
           v-if="false">
        <div class="drawerBtn"
             @click="updateDrawer1()">
          <i class="el-icon-arrow-right"></i>
        </div>
        <el-drawer size="350px"
                   customClass="amap-drawer"
          ...
          <div class="amap-main-content"
            ...
            <el-collapse-transition>
              <div class="echarts-box"
                ...
                <i class="iconfont law-delete1 right"
                   @click="status2 = false"></i>
                <div class="amap-chart">
                  <el-table v-loading="loading"
                    ...
                    <el-table-column prop="vehicleNumber"
                                     label="车牌号">
                    </el-table-column>
                    <el-table-column prop="overload"
                                     label="超载率">
                    </el-table-column>
    ...
    <div class="amap-search"
      ...
      <div class="search-box-blue"
        ...
        <el-button icon="el-icon-search"
                   @click="searchAllByBtn"></el-button>
      </div>
        ...
        <div class="amap-tool-search"
          ...
          <el-tree v-if="showTree"
            ...
            <span class="custom-tree-node"
              ...
              <span><img class="tree-node-icon"
                     :src="'./static/images/img/lawSupervise/'+data.icon+'.png'">{{data.label}}</span>
            </span>
            <!-- <span class="custom-tree-node">
...
export default {
  ...
  methods: {
    ...
    handleNodeClick (node) {
      ...
      // this.radioVal = '全选';
      //执法人员或者查询执法人员后选择执法人员
      if ((node.username && !node.icon) || (node.icon === 'icon_jc11' && node.label === '执法人员')) {
        let resultList = [];
