      ...
      <div class="handlePart">
        <el-form :inline="true" :model="userForm" ref="userFormRef">
          ...
          <el-form-item label="" label-width="13px">
            <el-button title="搜索" class="commonBtn searchBtn" size="medium" icon="iconfont law-sousuo" @click="enfPageList"></el-button>
            ...
            <el-button title="删除" class="commonBtn searchBtn" icon="el-icon-delete" size="medium" @click="deleteEnf"></el-button>
          </el-form-item>
        </el-form>
      </div>
      <div class="tablePart">
...
export default {
  ...
  methods: {//
    deleteEnf(row){
      ...
      _this.$confirm("确定要删除该执法号段吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
          _this.$store.dispatch("getEnfDeleteAll", _this.selectUserIdList[0]).then(
            res => {
            if(res.code===200){
              _this.$message({
                type: "success",
    ...
    enfPageList() {
      ...
      _this.$store.dispatch("getEnfList", data).then(res => {
    ...
    handleSizeChange(val) {//更改每页显示的条数
      let _this = this 
      _this.pageSize = val;
      _this.enfPageList();
    },
    handleCurrentChange(val) {//更换页码
      let  _this = this 
