  ...
  <div class="com_searchAndpageBoxPadding">
    <div class="searchPage">
      <div class="handlePart">
        <el-form class="search-form" :inline="true" :model="personForm" label-width="68px"  ref="personFormRef">
          <div>
            <div class="item">
              ...
              <el-form-item style="margin-top:1px;">
                ...
                <el-button size="medium" class="commonBtn toogleBtn" :title="isShow? '点击收缩':'点击展开'" :icon="isShow? 'iconfont law-top': 'iconfont law-down'" @click="isShow = !isShow" ></el-button>
              </el-form-item>
            </div>
            <div class="item" v-show="isShow">
              ...
              <el-form-item label="执法领域" prop="branchId">
                <el-select v-model="personForm.branchId" placeholder="执法领域" remote  @focus="getDepatements('执法门类','branchIdsInfo')">
                  <el-option
              ...
              <el-form-item label="岗位" prop="stationId">
                <el-select v-model="personForm.stationId" placeholder="选择岗位" remote  @focus="getStationInfo('人员信息-岗位','stationInfo')">
                  <el-option
              ...
              <el-form-item label="岗位状态" prop="stationStatus">
                <el-select v-model="personForm.stationStatus" placeholder="选择岗位状态" remote  @focus="getDepatements('人员信息-岗位状态','stationStatusInfo')">
                  <el-option
                    v-for="value in stationStatusInfo" :key="value.id" :label="value.name" :value="value.id">
                  </el-option>
                </el-select>
              </el-form-item>
            </div>
      ...
      <div class="tableHandle">
        <el-button type="primary"  icon="el-icon-plus" size="medium" @click="getDetailInfo('','1')">新增人员</el-button>
        ...
        >
          ...
          <el-table-column prop="stationStatusName" label="岗位状态" align="center">
            <template slot-scope="scope">
              <span v-if="scope.row.stationStatusName === '调动中'" style="color: #0074F5;">{{scope.row.stationStatusName}}</span>
              <span v-else-if="scope.row.stationStatusName === '退休'" style="color: #999;">{{scope.row.stationStatusName}}</span>
              <span v-else-if="scope.row.stationStatusName === '调离'" style="color: #E84241;">{{scope.row.stationStatusName}}</span>
          ...
          <el-table-column prop="opt" label="操作" align="center">
            <template slot-scope="scope">
              <el-button type="text"  @click="getDetailInfo(scope.row,'0')"><i class="iconfont law-eye" /></el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  methods:{
    applyTx(){//申请退休
      ...
      if(_this.selectUserIdList==undefined || _this.selectUserIdList.length==0){
        ...
        if(ssn==='调动中' || ssn==='退休' || ssn==='调离'){
          _this.$message({ type:'warning', message:'当前岗位状态为'+ssn+'，不允许退休'});
          return;
        }
        if(ssn==='在岗'){
          _this.$confirm('确认申请退休吗？退休后执法证号将被注销！','提示',{
            ...
            _this.$store.dispatch("applyTx", _this.selectUserIdList).then(
              res => {
                if(res.code === 200){
                  _this.$message({ type: "success", message: "申请退休成功!"});
                  //重新加载页面数据
                  _this.getPersonList();
                }else{
                  _this.$message({ type: "error", message: "申请退休失败!"});
                }
              }, err => {_this.$message({ type: "error", message: err.msg || '' });});
          });
        }
      }
    },
    applyDl(){//申请调离
      ...
      if(_this.selectUserIdList==undefined || _this.selectUserIdList.length==0){
        ...
        let ssn=_this.selectList[0];
        if(ssn==='调动中' || ssn==='退休' || ssn==='调离'){
          _this.$message({ type:'warning', message:'当前岗位状态为'+ssn+'，不允许调离'});
          return;
        }
        if(ssn==='在岗'){
