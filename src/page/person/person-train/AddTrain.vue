<template>
    <el-dialog
        ...
        width="38%"
    >
        <el-form :model="addTrainForm" label-position="right"  label-width="130px"  ref="addTrainForm" :rules="rules">
            <el-row>
            ...
            <el-row>
                <el-form-item label="培训类型:" prop="trainType">
                    <el-select v-model="addTrainForm.trainType" placeholder="请输入培训类型" @change="selectVal($event)" @focus="getDictInfo('人员信息-培训类型')">
                        <el-option
                            v-for="value in trainTypeInfo" :key="value.id" :label="value.name" :value="value.id">
                        </el-option>        
                    </el-select>
                </el-form-item>
            </el-row>
            ...
            <el-row>
                <el-form-item label="培训开始时间:" prop="trainBeginDate">
                    <el-date-picker v-model="addTrainForm.trainBeginDate" format="yyyy-MM-dd" value-format="yyyy-MM-dd"
                    placeholder="请选择培训开始时间" clearable></el-date-picker>
                </el-form-item>
            </el-row>
            <el-row>
                <el-form-item label="培训结束时间:" prop="trainEndDate">
                    <el-date-picker v-model="addTrainForm.trainEndDate" format="yyyy-MM-dd" value-format="yyyy-MM-dd"
                    placeholder="请选择培训结束时间" clearable></el-date-picker>
                </el-form-item>
            </el-row>
            <!-- <el-row>
  ...
  props: {
    ...
    data(){
        return{
            ...
            rules:{
                trainType: [{ required: true, message: "培训类型必须填写", trigger: "change" }],
                trainBeginDate: [{ required: true, message: "培训时间必须填写", trigger: "blur" }],
                trainEndDate: [{ required: true, message: "培训时间必须填写", trigger: "blur" }],
                trainName: [{ required: true, message: "培训名称必须填写", trigger: "blur" }],
                trainResult: [{ required: true, message: "培训结果必须填写", trigger: "blur" }],
            },
            ...
            _this.$refs.addTrainForm.validate((valid) => {
                if (valid) {
                    if(_this.handelType==1){
                        _this.$store.dispatch("addTrainInfoMoudle", _this.addTrainForm).then(res => {
                        ...
                        _this.$store.dispatch("updateTrainInfoMoudle", _this.addTrainForm).then(res => {
                            _this.$emit("getTrainList");
                                _this.$message({
                                    type: "success",
                                    message:  "修改成功!",
