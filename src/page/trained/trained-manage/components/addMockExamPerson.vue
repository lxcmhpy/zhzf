...
<script>
import { getAllTePersonList,addMockExamPerson } from '@/api/trained';
  ...
  methods: {
    ...
    submit() {
      if(this.selectUserIdList.length === 0){
        this.$message({ type: 'info', message: '请选择需要添加的人员' });
        return false;
      }
      const loading = this.$loading({
        ...
        let data = {
          teId: this.addExamPersonForm.teId,
          selectedPersonIds: this.selectUserIdList.join(',')
        };
        let data1 = {
          examPerson: JSON.stringify(data)
        };
        addMockExamPerson(data1).then(
          res => {
            loading.close();
            if (res.code == 200) {
              this.$emit("getExamPerson");
              this.closeDialog();
            }
          },
          err => {
    ...
    showModal(teId) {
      this.visible = true;
      this.currentPage = 1;
      this.addExamPersonForm.teId = teId;
    ...
    getPersonAllInfo() {
      ...
      getAllTePersonList(data).then(res => {
