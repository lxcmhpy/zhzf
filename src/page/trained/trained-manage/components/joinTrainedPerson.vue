<template>
  <div>
              ...
              <div class="item">
                ...
                <el-form-item>
                  <el-button
                    ...
                    @click="currentPage = 1;getJoinTrainedPerson();"
                  ></el-button>
                  <el-button
                    title="重置"
                    class="commonBtn searchBtn"
    ...
    <div>
      <addTrainedPerson ref="addTrainedPersonCompRef" @getExamPersonInfo="getJoinTrainedPerson"></addTrainedPerson>
    </div>
  </div>
</template>
<script>
import addTrainedPerson from "./addTrainedPerson";
import { getBatchPersonList, deleteTrainPerson,exportTrainPerson } from '@/api/trained';
...
export default {
  ...
  methods: {
    showModal(data) {
      ...
      this.getJoinTrainedPerson();
    },
    // 新增人员
    getAddPersonInfo() {
      this.$refs.addTrainedPersonCompRef.showModal(this.examPersonForm.trainId);
    },
    // 移除人员
    deleteAddPerson() {
      if (
    ...
    // 获取选中的user
    selectUser(val) {
      this.selectUserIdList = val.map(item => item.trainperId);
    },
    // 页面列表，性别翻译
    sexFormat(row) {
      if (row.sex === '0') {
    ...
    exportTrainPersonList(){
      ...
      const loading = this.$loading({
       ...
       exportTrainPerson(data).then(res => {
        loading.close()
